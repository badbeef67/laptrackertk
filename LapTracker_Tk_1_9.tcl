...
variable sep
foreach line $cars {
	set carlog [::html::openTag style]
	append carlog "p.serif \{font-family\: \"Courier New\", Courier, monospace\} p \{ white-space\: pre-wrap\;\}"
	append carlog [::html::closeTag]
	variable $line
	set log [open $line.html a+]
	set style [::html::openTag p]
	set style [string replace $style 2 2 " class=\"serif\"\;\>"]
	append carlog $style
	append carlog "[set ${line}(log)]\n$sep"
	append carlog [::html::closeTag]
	puts $log "$carlog"
	close $log
}
set racetime [clock format [clock seconds] -format "%y-%m-%d-%y_%H%M%S"]
set racesum [open _RaceSummary_.html a+]
set racelog [::html::openTag style]
append racelog "p.serif \{font-family\: \"Courier New\", Courier, monospace\} p \{ white-space\: pre-wrap\;\}"
append racelog [::html::closeTag]
set style [::html::openTag p]
set style [string replace $style 2 2 " class=\"serif\"\;\>"]
append racelog $style
append racelog "$racetime\n[.racestatus get 1.0 end]\n\n\n\n"
append racelog [::html::closeTag]
puts $racesum $racelog
close $racesum
}
